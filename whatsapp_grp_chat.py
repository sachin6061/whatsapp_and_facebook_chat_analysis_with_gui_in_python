...
import sqlite3
import pandas as pd
from whats_app_analysis import whatsapp_grp


class whatsapp_grp_chat_win(QtWidgets.QDialog):
    def __init__(self, parent, path):
        super().__init__(parent)
        self.msgbox = components.add_messagebox(self)
        self.msgbox.hide()
        self.analysis = whatsapp_grp()
        self.analysis.read_file(path)
        # print('Total group members', len(self.analysis.df.Author.unique()))
        ...
        self.header = components.header(self.header_frame, "WhatsApp Group Chat Analysis", self.hei_part)
        ...
        self.top_most_active = components.add_textarea(self.tab1, self.wid_part * 2, self.hei_part * 3,
                                                       self.wid_part * 4,
                                                       self.hei_part * 5)
        self.top_most_active.setPlainText(str(txt))

        txt = self.analysis.find_top_5_media()
        ...
        txt = self.analysis.find_top_links()
        ...
        # self.top_most_active.setPlainText("dshskjjka")

        txt = self.analysis.link_send_by_author()
        ...
        txt = self.analysis.find_top_5_emoji()
        ...
        txt = self.analysis.top5mostemojiuser()

        self.top_most_emoji_user = components.add_textarea(self.tab1, self.wid_part * 12, self.hei_part * 9,
                                                           self.wid_part * 4, self.hei_part * 5)
        self.top_most_emoji_user.setPlainText(str(txt))

        self.sel_y_lbl = components.add_lbl(self.tab3, self.wid_part * 11, self.hei_part, "Select Year -",
                                            self.wid_part * 2,
                                            40)
        ...
        self.lyears = components.add_listbox(self.tab3, self.wid_part * 13, self.hei_part, self.wid_part * 2, 40)
        ...
        components.set_image(self, 'linechartpermonth.png', self.monthchartlbl)
    ...
    def show_tab3(self):
        try:
            ...
            components.set_image(self, 'barchartpermonth.png', self.monthbarchartlbl)
    ...
    def show_tab4(self):
        try:
            ...
            components.set_image(self, 'activedaysmonth.png', self.activedayslbl)
            components.set_image(self, 'activehoursmonth.png', self.activehourslbl)
